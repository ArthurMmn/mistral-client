defmodule MistralClient.Client do
  ...
  def api_patch(url, params \\ [], config) do
    body =
      params
      |> Enum.into(%{})
      |> Jason.encode!()
    ...
    url
    |> patch(body, request_headers(config), request_options(config))
    |> handle_response()
  end

  def api_put(url, params \\ [], config) do
    body =
      params
      |> Enum.into(%{})
      |> Jason.encode!()
    ...
    url
    |> put(body, request_headers(config), request_options(config))
    |> handle_response()
  end
  ...
  def multipart_api_post_with_filename(url, file_path, file_param, filename, params, config) do
    body_params = params |> Enum.map(fn {k, v} -> {Atom.to_string(k), v} end)

    body = {
      :multipart,
      [
        {:file, file_path, {"form-data", [{:name, file_param}, {:filename, filename}]}, []}
      ] ++ body_params
    }
